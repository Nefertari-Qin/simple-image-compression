...
private:
	...
	// Private helpers for constructor stats(PNG & img)
	void initializeTwoDimSumVecs(int width, int height);
	void setSumAtXYOf(unsigned int x, unsigned int y, vector<vector<long>> & matrix, long crntValue);


	// Private helpers for getSum and getSumsq
	long getSumOfSquareAtXYWith(int x, int y, vector<vector<long>> const & matrix, int sideLength);
